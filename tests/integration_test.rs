use bollard::Docker;
use bollard::image::{PushImageOptions, RemoveImageOptions, TagImageOptions};
...
async fn test_docker_push_pull() {
    ...
    docker
        .tag_image(
            ...
            Some(TagImageOptions {
                repo: format!("localhost:{}/test-image", port),
    ...
    assert!(
        images.iter().any(|img| {
            img.repo_tags.iter().any(|tag| tag == &registry_image)
        }),
